...
class CronPoolData:
    ...
    def total_borrow(self) -> float:
        ...
        return round(self.tvl * self.utilization, 2)
    
    @property
    def available_liquidity(self) -> float:
        """Calculate available liquidity"""
        return round(self.tvl - self.total_borrow, 2)
...
def calculate_borrow_apr(utilization: float, protocol: str) -> float:
    ...
    base_rate = params["base_rate"] / 1e27
    slope1 = params["slope1"] / 1e27
    slope2 = params["slope2"] / 1e27
    kink = params["kink"] / 1e27 if params["kink"] > 1 else params["kink"]
    
    if utilization <= kink:
        # Below kink: base + slope1 * (U / kink)
        if kink > 0:
            borrow_apr = base_rate + slope1 * (utilization / kink)
        else:
            borrow_apr = base_rate
    else:
        # Above kink: base + slope1 + slope2 * ((U - kink) / (1 - kink))
        borrow_apr = (base_rate + slope1 + 
                     slope2 * ((utilization - kink) / (1 - kink)))
...
def debug_apr_calculations():
    """Debug function to trace APR calculation steps"""
    print("\n🔍 DEBUGGING APR CALCULATIONS")
    print("="*60)
    
    # 1. Check raw data from contracts
    print("\n1. Raw Contract Data:")
    print("-"*50)
    ...
    try:
        usde_address = web3.to_checksum_address("0x5d3a1Ff2b6BAb83b63cd9AD0787074081a52ef34")
        reserve_data = hyperlend_data_provider_contract.functions.getReserveData(usde_address).call()
        print(f"Reserve Data: {reserve_data}")
        print(f"Liquidity Rate (raw): {reserve_data[5]}")
        print(f"Current Variable Borrow Rate (raw): {reserve_data[4]}")
        ...
        slope2 = irm_contract.functions.getVariableRateSlope2(usde_address).call()
        kink = irm_contract.functions.getOptimalUsageRatio(usde_address).call()
        print(f"IRM Params - Base: {base_rate}, Slope1: {slope1}, Slope2: {slope2}, Kink: {kink}")
    except Exception as e:
        print(f"Error getting HyperLend data: {e}")
    
    # HyperFi data
    print("\nHyperFi:")
    try:
        usde_address = web3.to_checksum_address("0x5d3a1Ff2b6BAb83b63cd9AD0787074081a52ef34")
        reserve_data = data_provider_contract.functions.getReserveData(usde_address).call()
        print(f"Reserve Data: {reserve_data}")
        print(f"Liquidity Rate (raw): {reserve_data[5]}")
        ...
        print(f"Error getting HyperFi data: {e}")
    
    # 2. Check utilization calculations
    print("\n2. Utilization Calculations:")
    print("-"*50)
    hyperlend_data = get_hyperlend_yields_and_tvl()
    hypurrfi_data = get_hypurrfi_yields_and_tvl()
    print(f"HyperLend USDe: {hyperlend_data.get('USDe', {})}")
    print(f"HyperFi USDe: {hypurrfi_data.get('USDe', {})}")
    
    # 3. Check APR calculation steps
    print("\n3. APR Calculation Steps:")
    print("-"*50)
    
    # HyperLend example
    if 'USDe' in hyperlend_data:
        util = hyperlend_data['USDe']['utilization']
        print(f"\nHyperLend USDe (Utilization: {util:.4f})")
        borrow_apr = calculate_borrow_apr(util, "HyperLend")
        print(f"Calculated Borrow APR: {borrow_apr*100:.6f}%")
        supply_apr = calculate_supply_apr(util, "HyperLend")
        print(f"Calculated Supply APR: {supply_apr*100:.6f}%")
        
        # Show calculation steps
        params = PROTOCOL_PARAMS["HyperLend"]
        print("\nCalculation Breakdown:")
        if util <= params["kink"]:
            print(f"Below kink ({params['kink']:.4f}): base + slope1 * (U/kink)")
            print(f"{params['base_rate']:.6f} + {params['slope1']:.6f} * ({util:.6f}/{params['kink']:.6f})")
        else:
            print(f"Above kink ({params['kink']:.4f}): base + slope1 + slope2 * ((U-kink)/(1-kink))")
            print(f"{params['base_rate']:.6f} + {params['slope1']:.6f} + {params['slope2']:.6f} * (({util:.6f}-{params['kink']:.6f})/(1-{params['kink']:.6f}))")
        
        print(f"\nSupply APR = Borrow APR * U * (1 - reserve_factor)")
        print(f"{borrow_apr:.6f} * {util:.6f} * {1-params['reserve_factor']:.6f}")
    
    # HyperFi example
    if 'USDe' in hypurrfi_data:
        util = hypurrfi_data['USDe']['utilization']
        print(f"\nHyperFi USDe (Utilization: {util:.4f})")
        borrow_apr = calculate_borrow_apr(util, "HyperFi")
        print(f"Calculated Borrow APR: {borrow_apr*100:.6f}%")
        supply_apr = calculate_supply_apr(util, "HyperFi")
        print(f"Calculated Supply APR: {supply_apr*100:.6f}%")
        
        # Show calculation steps
        params = PROTOCOL_PARAMS["HyperFi"]
        print("\nCalculation Breakdown:")
        if util <= params["kink"]:
            print(f"Below kink ({params['kink']:.4f}): base + slope1 * (U/kink)")
            print(f"{params['base_rate']:.6f} + {params['slope1']:.6f} * ({util:.6f}/{params['kink']:.6f})")
