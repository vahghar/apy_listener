...
def save_result_to_db(result):
    conn = sqlite3.connect("optimizer_data.db")
    cursor = conn.cursor()
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS optimizer_results (
            ...
            to_protocol TEXT,
            amount_usd REAL,
            current_apr_from REAL,
            current_apr_to REAL,
            projected_apr REAL,
            utilization_from REAL,
            utilization_to REAL,
            extra_yield_bps REAL,
            notes TEXT
        )
    """)
    ...
    cursor.execute("""
        INSERT INTO optimizer_results (
            timestamp, from_protocol, to_protocol, amount_usd,
            current_apr_from, current_apr_to, projected_apr,
            utilization_from, utilization_to, extra_yield_bps, notes
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        result["from_protocol"],
        result["to_protocol"],
        result["amount_usd"],
        result["current_apr_from"],
        result["current_apr_to"],
        result["projected_apr"],
        result["utilization_from"],
        result["utilization_to"],
        result["extra_yield_bps"],
        result.get("notes", "")
    ))
...
"""
    ...
    if apr_check.get('move_recommended'):
        ...
        print("=" * 70)
        print(f"Move ALL ${apr_check['amount']:,.2f} from {apr_check['from']} to {apr_check['to']}")
        print(f"APR Difference: {apr_check['apr_difference']:.2f}%")
        print(f"Estimated Annual Gain: ${apr_check['annual_gain']:,.2f}")
        print("=" * 70)
        save_result_to_db({
    "from_protocol": apr_check['from'],
    "to_protocol": apr_check['to'],
    "amount_usd": apr_check['amount'],
    "current_apr_from": pools[apr_check['from']].current_apr,
    "current_apr_to": pools[apr_check['to']].current_apr,
    "projected_apr": pools[apr_check['to']].current_apr,
    "utilization_from": pools[apr_check['from']].utilization,
    "utilization_to": pools[apr_check['to']].utilization,
    "extra_yield_bps": (apr_check['apr_difference'] * 100),
    "notes": "Full move due to large APR difference"
        ...
        save_result_to_db({
    "from_protocol": "HyperFi",
    "to_protocol": "HyperLend",
    "amount_usd": result["amount"],
    "current_apr_from": result["current_apr"]["HyperFi"],
    "current_apr_to": result["current_apr"]["HyperLend"],
    "projected_apr": result["new_weighted_apr"],
    "utilization_from": result["current_util"]["HyperFi"],
    "utilization_to": result["current_util"]["HyperLend"],
    "extra_yield_bps": result["gain_bps"],
    "notes": "Optimized partial move"
})


        # Show detailed breakdown if optimization was performed
    ...
    "from_protocol": "HyperFi",
    "to_protocol": "HyperLend",
    "amount_usd": result2["amount"],
    "current_apr_from": result2["current_apr"]["HyperFi"],
    "current_apr_to": result2["current_apr"]["HyperLend"],
    "projected_apr": result2["new_weighted_apr"],
    "utilization_from": result2["current_util"]["HyperFi"],
    "utilization_to": result2["current_util"]["HyperLend"],
    "extra_yield_bps": result2["gain_bps"],
    "notes": "Optimized partial move"
})